...
const DynamicCalendar = ({
  ...
  // Live-resize override: mirrors the canvas calendar resize into the footer thumbnail.
  ...
  const cellMargin = calendarSetings.cellMargin ? calendarSetings.cellMargin : 0;
  const startMonth = parseInt(calendarSetings.startMonth);
  const startYear = parseInt(calendarSetings.startYear);
  const { month, year } = getMonthYear(startMonth, startYear, calendarMonthOffset !== undefined ? calendarMonthOffset : pageIndex * calendarCount + (calIndex - 1));

  const weeks = generateCalendarData(month, year, calendarSetings.language ? calendarSetings.language : "en");
  const dayNames = getDayNames(calendarSetings.language ? calendarSetings.language : "en");

  const alternativeColor = true;
  alternateColor = calendarSetings.alternativeBgColor ? calendarSetings.alternativeBgColor : "#FFFFFF";
  textColor = calendarSetings.textColor ? calendarSetings.textColor : "#000000";
  headerColor = calendarSetings.headerBgColor ? calendarSetings.headerBgColor : "#cccccc";
  headerTextColor = calendarSetings.headerTextColor ? calendarSetings.headerTextColor : "#000000";
  const weekendDayNameTextColor = calendarSetings.weekendTextColor ? calendarSetings.weekendTextColor : headerTextColor;
  const weekendTextColor = calendarSetings.weekendTextColor ? calendarSetings.weekendTextColor : textColor;
  fontSize = calendarSetings.fontSize ? calendarSetings.fontSize : 40;
  const fontFamily = calendarSetings.fontFamily ? calendarSetings.fontFamily : "Arial";
  const fontWeight = calendarSetings.fontWeight ? calendarSetings.fontWeight : "normal";
  const borderWidth = calendarSetings.borderWidth ? calendarSetings.borderWidth : 1;
  borderColor = calendarSetings.borderColor ? calendarSetings.borderColor : "#000000";
  const borderRadius = calendarSetings.borderRadius ? calendarSetings.borderRadius : 0;

  // Multi-column geometry — row-major order
    ...
    >
          ...
          return (
            ...
            >
              <rect
                ...
                width={cellWidth}
                height={cellHeight}
                fill={index === 0 && calendarSetings.weekendBgColor ? calendarSetings.weekendBgColor : headerColor}
                stroke={borderColor}
              ...
              >
                ...
                {(calendarSetings.dayNameFormat === undefined || calendarSetings.dayNameFormat === "") && day.shortName}
              </text>
            </g>
          );
        })
      ...
      {weeks.map((week, weekIndex) => {
        ...
        return week.map((day, dayIndex) => {
          ...
          const y = (rowInCol + 1) * (cellHeight + cellMargin) + fontSize / 2 + borderWidth / 2;

          let fillColor =
            (dayIndex + weekIndex) % 2 === 0
              ? calendarSetings.backgroundColor ? calendarSetings.backgroundColor : "#FFFFFF"
              : alternateColor;
          if (!alternativeColor) {
            fillColor = calendarSetings.backgroundColor ? calendarSetings.backgroundColor : "#FFFFFF";
          }
            ...
            >
              ...
              {day && (
                <text
                  ...
                  fontWeight={fontWeight}
                >
                  {convertNumberToLanguage(day, calendarSetings.language ? calendarSetings.language : "en")}
                </text>
              )}
            </g>
          );
